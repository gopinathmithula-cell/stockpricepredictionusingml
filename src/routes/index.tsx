...
const marketSnapshot = [
  { symbol: "TCS.NS", name: "Tata Consultancy Services", last: "₹3,842.20", change: "+2.84%", volume: "1.84M", trend: "Strong" },
  { symbol: "AAPL", name: "Apple Inc.", last: "$227.16", change: "+1.21%", volume: "48.62M", trend: "Positive" },
  { symbol: "RELIANCE.NS", name: "Reliance Industries", last: "₹2,914.70", change: "-0.42%", volume: "3.12M", trend: "Neutral" },
  { symbol: "INFY.NS", name: "Infosys Limited", last: "₹1,684.35", change: "+1.76%", volume: "2.41M", trend: "Positive" },
];
        ...
        <section className="min-w-0 flex-1">
          ...
          <div className="mx-auto max-w-[1440px] px-5 py-7 md:px-8 md:py-9">
            ...
            <section className="mt-8"><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-muted-foreground">Pipeline status</p><h3 className="mt-2 font-display text-2xl font-bold tracking-tight">From raw data to useful signal</h3></div><Button variant="ghost" size="sm" className="hidden sm:inline-flex">View methodology <ArrowUpRight /></Button></div><div className="mt-5 grid gap-3 md:grid-cols-5">{[[Database, "Data collection", "2 years synced", "Complete"], [RefreshCw, "Preprocessing", "Clean & aligned", "Complete"], [Activity, "Feature engineering", "MA50 · MA200 · Returns", "Complete"], [Cpu, "Model training", model, "Complete"], [Target, "Price prediction", "Ready for next close", "Active"]].map(([Icon, title, text, state], index) => { const StepIcon = Icon as LucideIcon; return <div key={title as string} className="relative rounded-xl border border-border bg-card p-4 shadow-sm"><div className="flex items-center justify-between"><div className="flex size-9 items-center justify-center rounded-lg bg-secondary text-primary"><StepIcon className="size-4" /></div><span className="text-[10px] font-bold text-muted-foreground">0{index + 1}</span></div><h4 className="mt-4 text-sm font-bold">{title as string}</h4><p className="mt-1 min-h-8 text-xs leading-relaxed text-muted-foreground">{text as string}</p><div className="mt-4 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide text-chart-2"><span className="size-1.5 rounded-full bg-chart-2" /> {state as string}</div></div> })}</div></section>
             ...
             {activeView === "Market data" && <MarketDataView selectedStock={selectedStock} onSelectStock={(stock) => setSelectedStock(stock)} />}
...
function MarketDataView({ selectedStock, onSelectStock }: { selectedStock: Stock; onSelectStock: (stock: Stock) => void }) {
  return <div><ViewHeading eyebrow="Live market snapshot" title="Track the session at a glance" description="Compare the instruments used by the prediction workspace and select one to update the overview." /><div className="grid gap-4 md:grid-cols-3"><MetricCard label="Market breadth" value="68%" detail="Advancing tracked symbols" icon={TrendingUp} positive /><MetricCard label="Session volume" value="56.0M" detail="Across tracked instruments" icon={BarChart3} positive /><MetricCard label="Data freshness" value="2 min" detail="Last successful sync" icon={RefreshCw} positive /></div><section className="mt-4 overflow-hidden rounded-2xl border border-border bg-card shadow-sm"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-border p-5 md:p-6"><div><h3 className="font-display text-lg font-bold">Tracked instruments</h3><p className="mt-1 text-xs text-muted-foreground">NSE and NASDAQ market close data</p></div><Badge variant="secondary" className="gap-1.5"><span className="size-1.5 rounded-full bg-chart-2" /> Synced</Badge></div><div className="overflow-x-auto"><table className="w-full min-w-[680px] text-left text-sm"><thead className="bg-secondary/60 text-[11px] font-bold uppercase tracking-[0.12em] text-muted-foreground"><tr><th className="px-5 py-3 font-semibold">Instrument</th><th className="px-5 py-3 font-semibold">Last price</th><th className="px-5 py-3 font-semibold">Change</th><th className="px-5 py-3 font-semibold">Volume</th><th className="px-5 py-3 font-semibold">Signal</th><th className="px-5 py-3 text-right font-semibold">Action</th></tr></thead><tbody className="divide-y divide-border">{marketSnapshot.map((row) => { const positive = row.change.startsWith("+"); const isSelected = row.symbol === selectedStock; return <tr key={row.symbol} className={cn("transition-colors hover:bg-secondary/40", isSelected && "bg-secondary/30")}><td className="px-5 py-4"><div className="font-bold">{row.symbol}</div><div className="mt-1 text-xs text-muted-foreground">{row.name}</div></td><td className="px-5 py-4 font-display font-bold">{row.last}</td><td className={cn("px-5 py-4 font-semibold", positive ? "text-chart-2" : "text-chart-5")}>{row.change}</td><td className="px-5 py-4 text-muted-foreground">{row.volume}</td><td className="px-5 py-4"><Badge variant={row.trend === "Neutral" ? "outline" : "secondary"}>{row.trend}</Badge></td><td className="px-5 py-4 text-right"><Button variant={isSelected ? "secondary" : "outline"} size="sm" onClick={() => { if (row.symbol === "TCS.NS" || row.symbol === "AAPL" || row.symbol === "RELIANCE.NS") onSelectStock(row.symbol); }}>{isSelected ? <><Check className="size-3.5" /> Selected</> : "View"}</Button></td></tr>; })}</tbody></table></div></section></div>;
}
