...
function NavItem({ icon: Icon, label, active = false, onClick }: { icon: LucideIcon; label: string; active?: boolean; onClick: () => void }) { return <Button variant={active ? "default" : "ghost"} className="w-full justify-start gap-3 px-3 py-2.5 text-left text-sm font-medium" onClick={onClick}><Icon className="size-4" />{label}</Button>; }
